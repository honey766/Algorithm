...
int main()
{
	...
	//길이 n인 각각의 사이클(v) 내에서 n-1번 Swap하면 된다.
	//이 때, 사이클 내의 최소 원소인 b에 대해 b를 최대한 활용해 준다.
	//오름차순으로 값이 bcde이면 Swap은 (b,c), (b,d), (b,e)일 때 최소이다.
	//사이클 내에 없는 값(arr의 최소값, arrSort[0])을 불러오는 것이 최소일 수가 있다.
	//이 경우에 Swap은 (a,b), (a,c), (a,d), (a,e), (a,b)가 된다.
	//사이클 내에서 가장 작은 값이 b였으므로, b를 a로 바꿔도 논리는 바뀌지 않는다.
	for (vector<int> v : vec) {
		...
		for (i = 0; i < len - 1; i++) {
			k = sequenceArr[j]; //j번째에 가야 할 애(j번째로 작은 애)가 k에 있음
			Swap(arr[j], arr[k], sum);
			j = k;//이제 v 내에서 최소인 원소는 k번째에 위치해 있다.
		}
		...
		//최소 원소(arrSort[0])를 가져오는 것은 중간의 for을 감싸는 if문 2번이다. 
